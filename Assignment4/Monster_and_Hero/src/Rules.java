...
public class Rules {//Create Tic Tac Toe rules
    ...
    private boolean horizontalAdjacent;
    private boolean verticalAdjacent;
    ...
    public boolean If_Movement_Valid(int board_length, int board_width, int[][] pieces, int[][][][] walls, int[] piece_position, char movement) throws Exception {//Let the player give their choice of next step
        ...
        switch (movement) {
            case 'W':
                if (now_piece_row - 1 >= 0) { //If out of the board
                    if (walls[now_piece_row][now_piece_column][now_piece_row - 1][now_piece_column] == 0) {//If there is a wall
                        if (pieces[now_piece_row - 1][now_piece_column] != 0) { //If there is another piece
                            if (now_piece_row - 2 >= 0) {//If after jumping out of the board
                                ...
                                System.out.println("Can't move out of the board1!");
                                return false;
                            }
                        } else {
                            return true;
                    ...
                    System.out.println("Can't move out of the board2!");
                    return false;
                }
            case 'A':
            ...
            case 'S':
                if (now_piece_row + 1 < board_length) {
                    if (walls[now_piece_row][now_piece_column][now_piece_row + 1][now_piece_column] == 0) {//If there is a wall
                        if (pieces[now_piece_row + 1][now_piece_column] != 0) { //If there is another piece
    ...
    public boolean If_Placement_Valid(int board_length, int board_width, int[][][][] walls, int[] this_placement) throws Exception {//Let the player give their choice of next step
        ...
        for (int i = 0; i < this_placement.length; i += 2) {
            if (this_placement[i] - 1 >= board_length) {
                System.out.println("The wall is out of board!");
                return false;
            }
        }

        for (int i = 1; i < this_placement.length; i += 2) {
            if (this_placement[i] - 1 >= board_width) {
                System.out.println("The wall is out of board!");
                return false;
            }
        }

        for (int i = 0; i < this_placement.length; i += 4) {
            if (walls[this_placement[i] - 1][this_placement[i + 1] - 1][this_placement[i + 2] - 1][this_placement[i + 3] - 1] == 1) {
        ...
        if (((Math.abs(this_placement[0] - this_placement[2]) == 1 && this_placement[1] == this_placement[3]) ||
            ...
            if(this_placement[0] == this_placement[2]){
                horizontalAdjacent = (this_placement[4] == this_placement[6] && Math.abs(this_placement[4] - this_placement[2]) == 1 && this_placement[5] == this_placement[1] && this_placement[7] == this_placement[3]);
            }

            if(this_placement[1] == this_placement[3]){
                verticalAdjacent = (this_placement[5] == this_placement[7] && Math.abs(this_placement[5] - this_placement[3]) == 1 && this_placement[4] == this_placement[0] && this_placement[6] == this_placement[2]);
            }

            if (horizontalAdjacent || verticalAdjacent) {
                if(walls[this_placement[0] - 1][this_placement[1] - 1][this_placement[4] - 1][this_placement[5] - 1] == 1 &&
                        walls[this_placement[2] - 1][this_placement[3] - 1][this_placement[6] - 1][this_placement[7] - 1] == 1){
                    System.out.println("These two walls will be truncated by the existing walls, so they cannot be placed.!");
                    return false;
                }
                else {
                    return true;
                ...
                System.out.println("These two walls are not connected!");
                return false;
            }
        }
        else{
            System.out.println("Please enter valid positions of walls!");
            return false;
        }
    }
    ...
    public int[] QuoridorIf_Over(int[] piece_position, int board_length, int now_round) throws Exception {//Check if the Quoridor game is over
        result[0] = 0;
        if (now_round % 2 == 1){
    ...
    public boolean If_This_Round_Choice_Valid(int[] player_position, String[][] areas, char round_choice, int world_length, int world_width) throws Exception {//Check if the player's move is valid
        if (round_choice == 'W' || round_choice == 'w') {
            if(player_position[0] == 0){
                return false;
            }
            else if(areas[player_position[0] - 1][player_position[1]] == "2"){
                return false;
            }
        }
        if (round_choice == 'A' || round_choice == 'a') {
            ...
            else if(areas[player_position[0]][player_position[1] - 1] == "2"){
                return false;
            }
        }
        if (round_choice == 'S' || round_choice == 's') {
            if(player_position[0] == world_length - 1){
                return false;
            }
            else if(areas[player_position[0] + 1][player_position[1]] == "2"){
                return false;
        ...
        if (round_choice == 'D' || round_choice == 'd') {
            if(player_position[0] == world_width - 1){
                return false;
            }
            else if(areas[player_position[0]][player_position[1] + 1] == "2"){
                return false;
    ...
    public boolean BattleIf_Over(int number_of_team, Hero[] heroes, Monsters[] monsters){
        for (int i = 0; i < number_of_team; i++){
            if(heroes[i].Get_Hp() != 0 ){
                for (int j = 0; j < number_of_team; j++){
                    if(monsters[j].Get_HP() != 0 ){
                        return false;
                    }
                }
            }
